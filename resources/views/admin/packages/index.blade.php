    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
            <div class="bg-white shadow rounded-lg p-4 text-center">
                ...
                <p class="text-xl font-bold">{{ $totalPackages ?? 0 }}</p>
            </div>
            <div class="bg-white shadow rounded-lg p-4 text-center">
                            ...
                            @foreach ($packages as $package)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2 text-sm text-gray-500">
                                        {{ ucfirst($package->priority ?? 'standard') }}</td>
                                    <td class="px-4 py-2 text-sm">RM{{ number_format($package->shipping_cost ?? 0, 2) }}
                                    </td>
                                    <td class="px-4 py-2 text-sm text-gray-500">
                                        {{ $package->created_at->format('Y-m-d H:i') }}</td>
                                    <td class="px-4 py-2 text-sm space-x-2">
                                        ...
                                        @if (!in_array($package->package_status, ['delivered', 'in_transit']))
                                            <form method="POST"
                                                action="{{ route('admin.packages.destroy', $package->package_id) }}"
