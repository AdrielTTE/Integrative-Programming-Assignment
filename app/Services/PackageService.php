...
class PackageService
{
    ...
    public function processPackage(string $packageId): Package
    {
        ...
        // Only admins and the package owner can process
        if (!str_starts_with($user->user_id, 'AD') && $package->user_id !== $user->user_id) {
            throw new \Exception('Unauthorized: Cannot process this package');
        }
    ...
    public function cancelPackage(string $packageId, User $user = null): Package
    {
        ...
        if (!str_starts_with($user->user_id, 'AD')) {
            // Non-admins can only cancel their own packages
            if ($package->user_id !== $user->user_id) {
                throw new \Exception('Unauthorized: Cannot cancel package you do not own');
            }
        }
    ...
    {
        $package = Package::with(['user', 'delivery.driver', 'assignment', 'delivery.proofOfDelivery'])
     ...
     public function updatePackage(Package $package, array $data): Package
    {
        ...
        // Customers can only update certain fields of their own packages
        if (str_starts_with($user->user_id, 'C')) {
            if ($package->user_id !== $user->user_id) {
                throw new \Exception('Unauthorized: Cannot update package you do not own');
            }
            
            // Only allow specific fields for customer updates
            $allowedFields = ['package_contents', 'package_dimensions', 'sender_address', 'recipient_address', 'notes'];
            $filteredData = array_intersect_key($data, array_flip($allowedFields));
            $package->update($filteredData);
            return $package->fresh();
        }
        
        // Drivers cannot directly update packages (they use specific status update methods)
        if (str_starts_with($user->user_id, 'D')) {
            throw new \Exception('Drivers cannot directly update package details');
        }
        
        throw new \Exception('Unauthorized: Invalid user type');
    }
    ...
    public function getPackageHistory(string $packageId): array
    {
        ...
        // Authorization check with role-based access
